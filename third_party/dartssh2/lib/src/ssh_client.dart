...
class SSHClient {
  ...
  Future<SSHSession> shell({
    ...
    SSHX11Config? x11,
    Map<String, String>? environment,
    bool pipelinePtyAndShellRequests = false,
  }) async {
    await _authenticated.future;
    ...
    // RFC 4254 section 5.4 permits a client to send another channel request
    // before receiving the previous reply. Replies for the same channel are
    // required to arrive in request order, so PTY and shell results can still
    // be matched safely while saving one network round trip.
    if (pipelinePtyAndShellRequests && pty != null && x11 == null) {
      final ptyRequest = channelController.sendPtyReq(
        terminalType: pty.type,
        terminalWidth: pty.width,
        terminalHeight: pty.height,
        terminalPixelWidth: pty.pixelWidth,
      ...
      final shellRequest = channelController.sendShell();
      final results = await Future.wait([ptyRequest, shellRequest]);
