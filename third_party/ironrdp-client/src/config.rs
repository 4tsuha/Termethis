...
type StaticChannelFn =
    Arc<dyn Fn(&mut ironrdp_connector::ClientConnector, &PropertySet) + Send + Sync>;
...
impl ConfigBuilder {
    ...
    pub fn with_platform(
        mut self,
        platform: ironrdp_pdu::rdp::capability_sets::MajorPlatformType,
    ) -> Self {
    ...
    pub fn with_compression_type(
        mut self,
        ty: Option<ironrdp_pdu::rdp::client_info::CompressionType>,
    ) -> Self {
        self.compression_type = ty;
        if let Some(ty) = ty {
            self.properties
                .set_compression_level(level_from_compression_type(ty));
        } else {
    ...
    pub fn with_dvc_pipe_proxy(mut self, info: DvcProxyInfo) -> Self {
        ...
        self.properties
            .set_dvc_pipe_proxies(self.dvc_pipe_proxies.iter().map(|p| {
                ironrdp_cfg::DvcPipeProxy {
    ...
    {
        let cb: StaticChannelFn = Arc::new(
            move |connector: &mut ironrdp_connector::ClientConnector, ps| {
    ...
    pub fn missing(&self) -> Vec<MissingField> {
        ...
        if matches!(self.transport, TransportKind::RDCleanPath { .. })
            && self.rdcleanpath_token.is_none()
        {
    ...
    pub fn build(self) -> anyhow::Result<Config> {
        ...
        let codecs =
            client_codecs_capabilities(&codecs).map_err(|help| anyhow::anyhow!("{help}"))?;
    ...
    pub fn with_property_set(mut self, ps: &PropertySet) -> anyhow::Result<Self> {
        ...
        for (idx, proxy) in ps.dvc_pipe_proxies().enumerate() {
            let proxy =
                proxy.with_context(|| format!("invalid DVC pipe proxy spec at idx {idx}"))?;
...
fn compression_type_from_level(
    level: u32,
) -> anyhow::Result<ironrdp_pdu::rdp::client_info::CompressionType> {
