...
pub type RdpFrameCallback =
    Arc<dyn Fn(&[u8], NonZeroU16, NonZeroU16, InclusiveRectangle) + Send + Sync>;
...
pub struct RdpClient {
    ...
    frame_callback: Option<RdpFrameCallback>,
}
...
impl RdpClient {
    ...
    pub fn with_frame_callback(mut self, callback: RdpFrameCallback) -> Self {
            ...
            match out {
                ...
                ActiveStageOutput::GraphicsUpdate(region) => {
                    ...
                    if let Some(callback) = frame_callback {
                        callback(image.data(), width, height, region);
                        if !frame_announced {
                            output_event_sender
                                .send(RdpOutputEvent::FramePresented { width, height })
                                .await
                                .map_err(|e| {
