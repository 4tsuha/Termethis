...
use crate::csi_mode_scan::{CsiModeEvent, CsiModeScanner};
...
use crate::terminal_raster_present::{RasterChrome, RasterPresentFrame, TerminalRasterPresent};
use alacritty_terminal::grid::{Dimensions, Scroll};
use alacritty_terminal::index::{Boundary, Column, Direction, Line, Point, Side};
use alacritty_terminal::selection::{Selection, SelectionRange, SelectionType};
use alacritty_terminal::term::cell::{Cell, Flags};
use alacritty_terminal::term::search::{BuildError, Match, RegexIter, RegexSearch};
...
/// Compile a search pattern with [`SearchOptions`].
///
/// Case-insensitivity is applied via `RegexSearch::with_case_insensitive` — never
/// by embedding `(?i)` (that breaks `\b` whole-word boundaries under regex-syntax).
fn compile_search(pattern: &str, opt: &SearchOptions) -> Result<RegexSearch, Box<BuildError>> {
    ...
    // ASCII `\b` only: Unicode word boundaries are unsupported by alacritty's
    // lazy DFAs. Never embed `(?i)` here — case comes from SyntaxConfig.
    if opt.whole_word {
...
pub struct TerminalEngine {
    ...
    /// When false, `search_step` rejects wrap-around hits (GNOME "wrap off").
    search_wrap: bool,
    ...
    scroll_fraction: f64,
    /// Parallel sniffer for DEC private modes alacritty does not surface
    /// (2031 color-scheme, 2026 synchronized output). See [`CsiModeScanner`].
    csi_mode_scanner: CsiModeScanner,
    ...
    /// DEC 2026 synchronized output active. alacritty parses SyncUpdate as a
    /// no-op (no `TermMode` bit); we track it here and OR into `mode_flags`.
    synchronized_output_active: bool,
    /// Retained RGBA pixmap for the Rust-raster present path (Dart uploads to
    /// `ui.Image`). `None` until the first [`Self::take_raster_present`].
    raster: Option<TerminalRasterPresent>,
}
...
impl TerminalEngine {
    pub fn new(columns: u16, rows: u16, config: EngineConfig) -> TerminalEngine {
        ...
        TerminalEngine {
            ...
            scroll_fraction: 0.0,
            csi_mode_scanner: CsiModeScanner::new(),
    ...
    /// Library-owned synthetic bit (not from alacritty `TermMode`) — matches
    /// Dart `kModeSynchronizedOutput` (`1 << 23`).
    pub const MODE_SYNCHRONIZED_OUTPUT: u32 = 1 << 23;

    /// Whether DEC 2026 synchronized output is currently active.
    pub fn synchronized_output_active(&self) -> bool {
        self.synchronized_output_active
    }

    fn packed_mode_flags(&self) -> u32 {
        let mut flags = self.term.mode().bits();
        if self.synchronized_output_active {
            flags |= Self::MODE_SYNCHRONIZED_OUTPUT;
        }
        flags
    }
    ...
    pub fn advance(&mut self, bytes: Vec<u8>) {
        // Sniff modes alacritty does not surface (2031 / 2026). Use the original
        // bytes — OSC pre-filtering can splice the stream, but it only touches
        // `ESC ]` sequences, never the `ESC [` CSI we look for.
        let toggles = self.csi_mode_scanner.feed(&bytes);
        ...
        for toggle in toggles {
            match toggle {
                ...
                CsiModeEvent::ColorSchemeUnsubscribe => self.color_scheme_subscribed = false,
                CsiModeEvent::SyncUpdateSet => self.synchronized_output_active = true,
                CsiModeEvent::SyncUpdateReset => self.synchronized_output_active = false,
            }
        }
    }
    ...
    pub fn search_set(&mut self, pattern: String, opt: SearchOptions) -> bool {
        match compile_search(&pattern, &opt) {
            Ok(re) => {
                self.search = Some(re);
                self.search_wrap = opt.wrap;
    ...
    fn search_step(&mut self, direction: Direction) -> bool {
        ...
        // Always search with max_lines: None. wrap=false is enforced below —
        // do NOT pass Some(total_lines-1): alacritty filters that to None (no-op).
        let found = self.term.search_next(re, origin, direction, Side::Left, None);
        match found {
            Some(m)
                if self.search_wrap
                    || !Self::match_wrapped_past_origin(&m, origin, direction) =>
            {
    ...
    fn scroll_refresh(&mut self, line_delta: i32) -> RenderUpdate {
        ...
        RenderUpdate {
            ...
            cursor_shape,
            cursor_blinking,
            mode_flags: self.packed_mode_flags(),
    ...
    pub fn take_damage(&mut self) -> RenderUpdate {
        // Partial packs use viewport rows; history views still need a full snapshot.
        // Live-edge mid-cell (`scroll_fraction != 0`) can ride overscan + fraction
        // on a partial / scroll_refresh-style payload.
        if self.term.grid().display_offset() > 0 {
            self.term.reset_damage();
            return if self.search.is_some() {
                self.full_snapshot_searched()
            } else {
                self.full_snapshot()
            };
        }
        // Why: partial line packs omit search match flags; retain would keep
        // stale highlights on clean rows. Match after_scroll_refresh.
        if self.search.is_some() {
            self.term.reset_damage();
            return self.full_snapshot_searched();
        }
        ...
        let u = match damaged {
            ...
            Some(mut rows) => {
                rows.sort_unstable();
                rows.dedup();
                // pack_viewport_line ORs FLAG_SELECTED (line_cells did not).
                let mut lines: Vec<LineUpdate> = rows
                    .into_iter()
                    .map(|row| self.pack_viewport_line(row))
                    .collect();
                // Mid-cell at live edge: ship overscan like scroll_refresh(0).
                if self.scroll_fraction != 0.0 {
                    let screen_rows = self.term.screen_lines();
                    lines.push(self.overscan_line_update(screen_rows));
                }
                let (default_fg, default_bg, cursor_color) = self.chrome_colors();
                RenderUpdate {
                    lines,
    ...
    /// Consume damage like [`Self::take_damage`], rasterize into a retained RGBA
    /// pixmap, and return chrome + pixels (no columnar `LineUpdate` payload).
    ///
    /// Used by the Dart Rust-raster present path so MirrorGrid can apply
    /// chrome-only updates while selection/search still call full snapshots.
    pub fn take_raster_present(&mut self) -> RasterPresentFrame {
        let update = self.take_damage();
        self.apply_render_to_raster(&update)
    }

    /// Full viewport raster (resize / explicit refresh).
    pub fn full_raster_present(&mut self) -> RasterPresentFrame {
        let update = self.full_snapshot();
        self.apply_render_to_raster(&update)
    }

    /// Rasterize a [`RenderUpdate`] already produced by scroll/snapshot helpers.
    pub fn rasterize_update(&mut self, update: &RenderUpdate) -> RasterPresentFrame {
        self.apply_render_to_raster(update)
    }

    fn apply_render_to_raster(&mut self, update: &RenderUpdate) -> RasterPresentFrame {
        let cols = self.term.columns().max(MIN_COLUMNS) as u16;
        let rows = self.term.screen_lines().max(MIN_SCREEN_LINES) as u16;
        let cell_w = self.cell_w.max(1);
        let cell_h = self.cell_h.max(1);
        let need_new = match &self.raster {
            None => true,
            Some(r) => {
                r.width() != cols as u32 * cell_w as u32
                    || r.height() != rows as u32 * cell_h as u32
            }
        };
        if need_new {
            let mut buf = TerminalRasterPresent::new(cols, rows, cell_w, cell_h);
            buf.clear(update.default_bg);
            self.raster = Some(buf);
        }
        let raster = self.raster.as_mut().expect("raster just ensured");
        if update.full || need_new {
            raster.clear(update.default_bg);
        } else if update.scroll_line_delta != 0 {
            raster.rotate_rows(update.scroll_line_delta);
        }
        for line in &update.lines {
            if line.line == OVERSCAN_LINE_TAG {
                continue;
            }
            raster.blit_line(line);
        }
        let chrome = RasterChrome {
            cursor_line: update.cursor_line,
            ...
            mode_flags: update.mode_flags,
            display_offset: update.display_offset,
            history_size: update.history_size,
            scroll_fraction: update.scroll_fraction,
            default_fg: update.default_fg,
            default_bg: update.default_bg,
            cursor_color: update.cursor_color,
        };
        raster.into_frame(chrome, update.full || need_new)
    }
}
...
mod tests {
    ...
    #[test]
    fn mode_flags_pack_synchronized_output() {
        let mut e = engine(20, 5);
        assert!(!e.synchronized_output_active());
        assert_eq!(
            e.full_snapshot().mode_flags & TerminalEngine::MODE_SYNCHRONIZED_OUTPUT,
            0
        );

        e.advance(b"\x1b[?2026h".to_vec());
        assert!(e.synchronized_output_active());
        assert_ne!(
            e.full_snapshot().mode_flags & TerminalEngine::MODE_SYNCHRONIZED_OUTPUT,
            0
        ...
        e.advance(b"partial\x1b[?2026l".to_vec());
        assert!(!e.synchronized_output_active());
        assert_eq!(
            e.full_snapshot().mode_flags & TerminalEngine::MODE_SYNCHRONIZED_OUTPUT,
            0
    ...
    #[test]
    fn synchronized_output_split_across_advances() {
        let mut e = engine(20, 5);
        e.advance(b"\x1b[?20".to_vec());
        assert!(!e.synchronized_output_active());
        e.advance(b"26h".to_vec());
        assert!(e.synchronized_output_active());
        e.advance(b"\x1b[?2026l".to_vec());
        assert!(!e.synchronized_output_active());
    }
    ...
    fn search_set_marks_matches_and_focuses_first() {
        ...
        assert!(e.search_set("foo".to_string(), SearchOptions::default()));
    ...
    fn search_next_moves_focus_to_the_second_match() {
        let mut e = engine(20, 5);
        e.advance(b"foo bar foo".to_vec());
        e.search_set("foo".to_string(), SearchOptions::default());
    ...
    fn search_prev_walks_back_then_re_finds_the_first_match() {
        ...
        let mut e = engine(20, 5);
        e.advance(b"foo bar foo".to_vec());
        e.search_set("foo".to_string(), SearchOptions::default());      // first "foo" (col 0) focused
    ...
    fn invalid_regex_returns_false_and_highlights_nothing() {
        ...
        assert!(!e.search_set("(".to_string(), SearchOptions::default()));
        let u = e.full_snapshot_searched();
    ...
    #[test]
    fn search_literal_dot_does_not_match_any_char() {
        let mut e = engine(20, 5);
        e.advance(b"a.b axb".to_vec());
        let opt = SearchOptions {
            regex: false,
            ..SearchOptions::default()
        };
        assert!(e.search_set(".".to_string(), opt));
        let u = e.full_snapshot_searched();
        // Literal '.' at col 1 only — not every character.
        assert_ne!(u.lines[0].cell(1).flags & FLAG_MATCH, 0);
        assert_eq!(u.lines[0].cell(0).flags & FLAG_MATCH, 0);
        assert_eq!(u.lines[0].cell(4).flags & FLAG_MATCH, 0); // 'a' of axb
        assert_eq!(u.lines[0].cell(5).flags & FLAG_MATCH, 0); // 'x'
    }

    #[test]
    fn search_whole_word_skips_mid_token() {
        let mut e = engine(20, 5);
        e.advance(b"foobar foo".to_vec());
        let opt = SearchOptions {
            whole_word: true,
            ..SearchOptions::default()
        };
        assert!(e.search_set("foo".to_string(), opt));
        let u = e.full_snapshot_searched();
        // Standalone "foo" at col 7; not the prefix of "foobar".
        assert_eq!(u.lines[0].cell(0).flags & FLAG_MATCH, 0);
        assert_ne!(u.lines[0].cell(7).flags & FLAG_MATCH_CURRENT, 0);
    }

    #[test]
    fn search_case_sensitive_requires_exact_case() {
        let mut e = engine(20, 5);
        e.advance(b"Foo foo".to_vec());
        let insensitive = SearchOptions {
            case_sensitive: false,
            ..SearchOptions::default()
        };
        assert!(e.search_set("foo".to_string(), insensitive));
        let u = e.full_snapshot_searched();
        assert_ne!(u.lines[0].cell(0).flags & FLAG_MATCH, 0); // Foo
        assert_ne!(u.lines[0].cell(4).flags & FLAG_MATCH, 0); // foo

        let sensitive = SearchOptions {
            case_sensitive: true,
            ..SearchOptions::default()
        };
        assert!(e.search_set("foo".to_string(), sensitive));
        let u = e.full_snapshot_searched();
        assert_eq!(u.lines[0].cell(0).flags & FLAG_MATCH, 0); // Foo excluded
        assert_ne!(u.lines[0].cell(4).flags & FLAG_MATCH_CURRENT, 0);
    }

    #[test]
    fn search_wrap_false_does_not_wrap_past_end() {
        let mut e = engine(20, 5);
        e.advance(b"foo\r\nbar\r\nfoo".to_vec());
        let opt = SearchOptions {
            wrap: false,
        ...
        assert!(e.search_next()); // second "foo" on line 2
        let u = e.full_snapshot_searched();
        assert_ne!(u.lines[2].cell(0).flags & FLAG_MATCH_CURRENT, 0);

        // No wrap back to the first hit.
        assert!(!e.search_next());
        let u = e.full_snapshot_searched();
        assert_ne!(u.lines[2].cell(0).flags & FLAG_MATCH_CURRENT, 0);
        assert_eq!(u.lines[0].cell(0).flags & FLAG_MATCH_CURRENT, 0);
    ...
    fn scroll_pixels_accumulates_fraction_then_steps_a_line() {
        ...
        // Reverse half a cell from offset 1 → pos 0.5, within one line of the
        // live bottom, so edge-snap pins flush (see scroll_pixels_snaps_near_live_bottom).
        e.scroll_pixels(-9.0);
        let u = e.full_snapshot();
        assert_eq!(u.display_offset, 0);
    ...
    #[test]
    fn take_damage_keeps_partial_when_fraction_nonzero_at_live_edge() {
        // Live edge + mid-cell fraction: overscan + fraction ride on a partial
        // (scroll_refresh-style) payload — not a full viewport dump.
        let mut e = engine(10, 3);
        fill_lines(&mut e, 12);
        e.set_cell_pixels(9, 18);
        e.take_damage(); // drain
        ...
        let u = e.take_damage();
        assert!(!u.full, "live-edge fraction must not force full snapshot");
        assert_eq!(u.display_offset, 0);
        assert!((u.scroll_fraction - 0.5).abs() < 1e-9);
        assert!(
            u.lines.iter().any(|l| l.line == OVERSCAN_LINE_TAG),
            "overscan must ride with fraction"
        );
        assert!(
            u.lines.len() < e.term.screen_lines() + 1,
            "expected edge/partial payload, got {} lines",
            u.lines.len()
        );
    }

    #[test]
    fn take_damage_partial_preserves_selection_flags() {
        let mut e = engine(20, 3);
        e.advance(b"hello world".to_vec());
        e.take_damage(); // drain
        e.selection_start(0, 0, false, 0);
        e.selection_update(0, 4, true);
        // Further input damages the line while selection is still active.
        e.advance(b"!".to_vec());
        let u = e.take_damage();
        assert!(!u.full, "live-edge typing should stay partial");
        let row0 = u
            .lines
            .iter()
            .find(|l| l.line == 0)
            .expect("damaged viewport row 0");
        assert_ne!(row0.cell(0).flags & FLAG_SELECTED, 0);
        assert_ne!(row0.cell(4).flags & FLAG_SELECTED, 0);
    }

    #[test]
    fn take_damage_with_search_active_returns_searched_full() {
        let mut e = engine(20, 5);
        e.advance(b"foo bar foo".to_vec());
        e.take_damage(); // drain
        assert!(e.search_set("foo".to_string(), SearchOptions::default()));
        e.advance(b"z".to_vec());
        let u = e.take_damage();
        assert!(u.full, "search-active damage must full_snapshot_searched");
        assert_ne!(u.lines[0].cell(0).flags & FLAG_MATCH, 0);
        assert_ne!(u.lines[0].cell(0).flags & FLAG_MATCH_CURRENT, 0);
    }

    #[test]
    fn take_damage_forces_full_when_scrolled_into_history() {
        // Partial packs use absolute/viewport rows at live edge; history views
        // still need a full snapshot.
        let mut e = engine(10, 3);
        fill_lines(&mut e, 12);
        e.take_damage(); // drain
        e.scroll_lines(2);
    ...
    #[test]
    fn take_raster_present_returns_rgba_without_needing_line_mirror() {
        let mut e = engine(8, 2);
        e.set_cell_pixels(6, 8);
        e.advance(b"Hi".to_vec());
        let frame = e.take_raster_present();
        assert_eq!(frame.width, 8 * 6);
        assert_eq!(frame.height, 2 * 8);
        assert_eq!(frame.rgba.len(), (frame.width * frame.height * 4) as usize);
        // At least one non-bg pixel from the 'H'/'i' glyphs.
        let bg = frame.default_bg;
        let bg_r = ((bg >> 16) & 0xFF) as u8;
        let bg_g = ((bg >> 8) & 0xFF) as u8;
        let bg_b = (bg & 0xFF) as u8;
        let saw = frame.rgba.chunks_exact(4).any(|p| p[0] != bg_r || p[1] != bg_g || p[2] != bg_b);
        assert!(saw, "expected glyph pixels distinct from default bg");
    }
}
